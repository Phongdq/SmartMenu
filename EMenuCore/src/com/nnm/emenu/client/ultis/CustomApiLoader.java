...
/**
 * A wrapper for the <a href="https://developers.google.com/loader/">Google
 * Loader/a>.
 */
public class CustomApiLoader extends ApiLoader {
//	private static String HOSTNAME = GWT.getHostPageBaseURL() + "js";
	...
	public static void loadApi(final String api, final String version,
			Runnable onLoad, ApiLoaderOptions optionalSettings) {
		CustomApiLoader loader = new CustomApiLoader();

		// Set the onLoad callback into the
		assert (onLoad != null);
		ApiLoaderOptions settings = optionalSettings;
		// settings.setBaseDomain(GWT.getHostPageBaseURL() + "js");
		if (settings == null) {
	 ...
	 */
	private static native void loadApi(String api, String version,
	...
	// True if the JavaScript __gwt_charts_AjaxLoader_onLoad callback has
	// already run.
	 ...
	 *            API key value.
	 */
	public CustomApiLoader(String apiKey) {
		if (initialized == true) {
			return;
		}
		System.out.println("Create Custom ApiLoader1");
	...
	private boolean injectJsApi(String apiKey) {
		...
		String protocol = (Window.Location.getProtocol().equals("https:")) ? "https:"
				: "http:";
		String src = protocol + "//" + HOSTNAME + "/jsapi?" + key
